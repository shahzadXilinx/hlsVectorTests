...
static void read_input(uintVec *in, hls::stream<uintVec> &inStream,
                       int size) {
...
static void compute_add(hls::stream<uintVec> &inStream,
                        hls::stream<uintVec> &outStream, int inc,
                        int size) {
// Auto-pipeline is going to apply pipeline to this loop
execute:
  for (int i = 0; i < size; i++) {
...
static void write_result(uintVec *out,
                         hls::stream<uintVec> &outStream, int size) {
// Auto-pipeline is going to apply pipeline to this loop
mem_wr:
  for (int i = 0; i < size; i++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
...
/*
   ...
   */
void adder(uintVec *in, uintVec *out, int inc, int size) {
  ...
  // during sw emulation.
  static hls::stream<uintVec> inStream("input_stream");
  static hls::stream<uintVec> outStream("output_stream");
