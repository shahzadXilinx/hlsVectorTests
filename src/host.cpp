...
int main(int argc, char **argv) {
  ...
  size_t vector_size_bytes = sizeof(uintVec) * DATA_SIZE;
  std::vector<uintVec, aligned_allocator<uintVec>> source_input(DATA_SIZE);
  std::vector<uintVec, aligned_allocator<uintVec>> source_hw_results(DATA_SIZE);
  std::vector<uintVec, aligned_allocator<uintVec>> source_sw_results(DATA_SIZE);

  // Create the test data and Software Result
  unsigned int count=0;
  for (int i = 0; i < DATA_SIZE; i++) 
  {
    for (int j = 0; j < elementsPerVec; ++j)
    {

    source_input[i][j] = count;
    source_sw_results[i][j] = count + INCR_VALUE;
